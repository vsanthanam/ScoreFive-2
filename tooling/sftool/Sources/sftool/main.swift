...
enum Commands {
    ...
    static func runTests(_ root: String, tuistConfig: TuistConfiguration, name: String, os: String) throws -> String {
        try tuist(on: root, tuistConfig: tuistConfig) {
            try killXcode()
            try generate(on: root, tuistConfig: tuistConfig)
            return try shellOut(to: "xcodebuild -workspace \(root)/\(tuistConfig.root)/ScoreFive.xcworkspace -sdk iphonesimulator -scheme ScoreFive -destination 'platform=iOS Simulator,name=\(name),OS=\(os)' test")
        }
    }
    ...
    static func generate(on root: String, tuistConfig: TuistConfiguration) throws {
        try tuist(on: root, tuistConfig: tuistConfig) {
            let command = root + "/bin/tuist/tuist"
            let tuistDir = root + "/" + tuistConfig.root
    ...
    @discardableResult
    fileprivate static func tuist(on root: String, tuistConfig: TuistConfiguration, action: () throws -> String) throws -> String {
        let options = tuistConfig
            .generationOptions
            .map { "." + $0 }
        ...
        """
        let dir = root + "/" + tuistConfig.root + "/Tuist"
