...
open class CountingLabel: UILabel {
    ...
    public typealias OptionalCallback = (() -> Void)
    public typealias OptionalFormatBlock = (() -> String)

    public var completion: OptionalCallback?
    public var animationDuration: Duration = .brisk
    public var decimalPoints: DecimalPoints = .zero
    public var countingMethod: Method = .easeInOut
    public var customFormatBlock: OptionalFormatBlock?

    open func count(from: Float, to: Float, duration: Duration = .strolling) {
        startingValue = from
        destinationValue = to
        timer?.invalidate()
        timer = nil
    ...
    open func countFromCurrent(to: Float, duration: Duration = .strolling) {
        count(from: currentValue, to: to, duration: duration)
    }

    open func countFromZero(to: Float, duration: Duration = .strolling) {
